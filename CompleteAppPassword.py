...
def init_email():
    ...
    gmail_app_password = 'oxewdgyfhfsfsmdx'
    sent_to = ['noah.robit@gmail.com']
    return gmail_user,gmail_app_password,sent_to
...
def make_email(ty):
    ...
    #New begin
    sent_from = 'nialabtempaturesensor@gmail.com'
    sent_to = ['noah.robit@gmail.com']
    sent_subject = 'Tempature Alarm!' +  ' @ ' + dateInfo + ' on ' +  str(today.strftime("%b %d ")) 
    sent_body = 'Hello Rohin, the tempature in the room is: ' + str(ty)
...
def send_email(ty):
    gmail_user,gmail_app_password,sent_to = init_email()
    ...
    while True: # Main program Loop
        ...
        if ty>tempUB_F or ty<tempLB_F:
            ...
            if dx >= wtTime:
                ...
                send_email(ty) # why send tx? Before: send_email(tx,ty)
                print('>>>>>>>Email sent>>>>>>>>') #After
                txEmail = tx
            else:
                #print('>>>>>>>Email sent>>>>>>>>')  #Before
                print('Out of range, No email sent') #After
